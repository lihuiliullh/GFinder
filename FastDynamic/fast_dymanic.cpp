...
#include "query_augment_experiment.h"
...
inline bool is_contain_empty_indexset() {
	for (int step = 0; step < g_core_size; step++) {
		CPINode* tmp_node = &g_indexSet_idx_is_id[step];
...
int main(int argc, char *argv[])
{
	...
	argv[1] = "C:\\Users\\lihuil2\\Documents\\DARPA\\Milan\\complete-phone-sms-grouped.csv.format";
	argv[2] = "C:\\Users\\lihuil2\\Documents\\DARPA\\Milan\\template7-grouped.csv.format";
		...
		if (g_isTree) {
			...
			for (int region = 0; region < g_root_candidates_size; region++) {			
				...
				if (!DYNAMIC) {
					...
					disableSomeIndexSet_keepConnect(g_indexSet_idx_is_id, 32, 47, false);
 					//disableSomeIndexSet_keepConnect(indexSet, 0, 41, true);

					//disableSomeIndexSet_keepConnect(indexSet, 1, 20, true);
					...
					disableSomeIndexSet_keepConnect(g_indexSet_idx_is_id, 5, 27, false);
